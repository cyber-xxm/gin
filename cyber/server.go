...
type conn struct {
	...
	// 先写响应数据到缓存里面，然后再读出来通过base64编码返回
	bufs *bytes.Buffer
...
func (cw *chunkWriter) Write(p []byte) (n int, err error) {
	...
	if cw.chunking {
		...
		_, err = fmt.Fprintf(cw.res.conn.bufs, "%x\r\n", len(p))
		if err != nil {
	...
	n, err = cw.res.conn.bufs.Write(p)
	if cw.chunking && err == nil {
		// _, err = cw.res.conn.bufw.Write(crlf)
		_, err = cw.res.conn.bufs.Write(crlf)
	}
	if err != nil {
		cw.res.conn.rwc.Close()
	}
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	readString, err := c.bufr.ReadString('\n')
	if err != nil {
		c.server.logf("read tcp data err: %v", err)
		return nil, err
	}
	decodeString, err := base64.StdEncoding.DecodeString(readString)
	if err != nil {
		c.server.logf("decode data err: %v", err)
		return nil, err
	}
	r := bufio.NewReader(strings.NewReader(string(decodeString)))
	req, err := http.ReadRequest(r)
	req.Body = &body{src: io.LimitReader(r, req.ContentLength), closing: req.Close}
...
func (w *response) WriteHeader(code int) {
	...
	if code >= 100 && code <= 199 && code != http.StatusSwitchingProtocols {
		writeStatusLine(w.conn.bufw, w.req.ProtoAtLeast(1, 1), code, w.statusBuf[:], w.conn.bufs)

		// Per RFC 8297 we must not clear the current header map
		// w.handlerHeader.WriteSubset(w.conn.bufw, excludedHeadersNoBody)
		// w.conn.bufw.Write(crlf)
		// w.conn.bufw.Flush()
		w.handlerHeader.WriteSubset(w.conn.bufs, excludedHeadersNoBody)
		w.conn.bufs.Write(crlf)
		resp64 := base64.StdEncoding.EncodeToString([]byte(w.conn.bufs.String()))
		w.conn.bufw.WriteString(resp64)
		w.conn.bufw.Flush()
	...
	if cl := headerGet(w.handlerHeader, "Content-Length"); cl != "" {
...
func (h extraHeader) Write(w *bytes.Buffer) {
	if h.date != nil {
		w.Write(headerDate)
		w.Write(h.date)
		w.Write(crlf)
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	te := headerGet(header, "Transfer-Encoding")
	hasTE := te != ""
	...
	if w.handlerDone.Load() && !trailers && !hasTE && bodyAllowedForStatus(w.status) && !headerHas(header, "Content-Length") && (!isHEAD || len(p) > 0) {
		if w.contentLength == 0 || p == nil || len(p) == 0 {
			setHeader.contentLength = strconv.AppendInt(cw.res.clenBuf[:0], w.contentLength, 10)
		} else {
	...
	if w.wants10KeepAlive && keepAlivesEnabled {
		sentLength := headerGet(header, "Content-Length") != ""
		if sentLength && headerGet(header, "Connection") == "keep-alive" {
	...
	if headerGet(header, "Connection") == "close" || !keepAlivesEnabled {
		w.closeAfterReply = true
	}
	...
	delConnectionHeader := w.closeAfterReply &&
		(!keepAlivesEnabled || !hasToken(headerGet(cw.header, "Connection"), "close")) &&
	...
	cw.header.WriteSubset(w.conn.bufs, excludeHeader)
	//setHeader.Write(w.conn.bufw)
	setHeader.Write(w.conn.bufs)
	// w.conn.bufw.Write(crlf)
	w.conn.bufs.Write(crlf)
}
...
func writeStatusLine(bw *bufio.Writer, is11 bool, code int, scratch []byte, bufs *bytes.Buffer) {
	if is11 {
		// bw.WriteString("HTTP/1.1 ")
		bufs.WriteString("HTTP/1.1 ")
	} else {
		// bw.WriteString("HTTP/1.0 ")
		bufs.WriteString("HTTP/1.0 ")
	}
	if text := http.StatusText(code); text != "" {
		...
		// bw.WriteString(text)
		// bw.WriteString("\r\n")
		bufs.Write(strconv.AppendInt(scratch[:0], int64(code), 10))
		bufs.WriteByte(' ')
		bufs.WriteString(text)
		bufs.WriteString("\r\n")

	} else {
		// don't worry about performance
		// fmt.Fprintf(bw, "%03d status code %d\r\n", code, code)
		fmt.Fprintf(bufs, "%03d status code %d\r\n", code, code)
	}
}
...
func (w *response) write(lenData int, dataB []byte, dataS string) (n int, err error) {
	...
	if dataB != nil {
		...
		cl, err := w.conn.bufs.Write(dataB)
		w.contentLength = int64(cl)
		return cl, err
	} else {
		cl, err := w.conn.bufs.WriteString(dataS)
		w.contentLength = int64(cl)
...
func (w *response) finishRequest() {
	...
	data := w.conn.bufs.String()
	w.conn.bufs.Reset()
	w.cw.close()
	//w.conn.bufs.Write(crlf)
	w.conn.bufs.WriteString(data)
	resp64 := base64.StdEncoding.EncodeToString([]byte(w.conn.bufs.String()))
	w.conn.bufw.WriteString(resp64)
	putBufioWriter(w.w)
	w.w.Flush()
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		} else if headerGet(req.Header, "Expect") != "" {
...
func (srv *Server) shouldConfigureHTTP2ForServe() bool {
	...
	// See Issue 15908.
	return slices.Contains(srv.TLSConfig.NextProtos, "h2")
}
